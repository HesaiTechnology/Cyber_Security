...
class HttpsApi(object):
    ...
    def login(self, username, passwd):
        url = 'https://192.168.1.201/pandar.cgi?action=get&object=login'
    ...
    def send_get_request_and_receive_return_info(self,url,timeout=10, *, retry_num=1):
        ...
        s.mount('https://', a)
        res = s.request('GET', url, timeout = timeout, cookies=self.cookies, verify=False)
    ...
    def send_post_request_and_receive_return_info(self, url, timeout=10, *, retry_num=1, data=None, files=None):
        ...
        a = requests.adapters.HTTPAdapter(max_retries=retry_num)
        s.mount('https://', a)
        res = s.request('POST', url, data=data, files=files,timeout=timeout, cookies=self.cookies, verify=False)
        assert res.status_code == 200, 'cannot connect to lidar, error code: %s' %(res.status_code)
        info =  json.loads(res.text)
        info['Connection_Status'] = res.status_code
        return info
...
if __name__ == "__main__":
    ...
    print(y.send_get_request_and_receive_return_info("https://192.168.1.201/pandar.cgi?action=get&object=workmode"))
